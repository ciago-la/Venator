// === Altervenator v13s7c — perfiles (guardar/cargar/exportar/importar) + diaria 1x/día + urgentes 10% programadas ===
(function(){
  const VER='v13s7c';
  ...
  // Iconos mínimos (mismos que v13s7b)
  var ICONS={
  ...
  function renderShop(){
    const SC=shopConsumibles, SE=shopEsteticos, IL=inventoryList;
    if (!SC || !SE || !IL) return;
    SC.innerHTML=''; SE.innerHTML=''; IL.innerHTML='';
  ...
  document.addEventListener('click', function(e){
    ...
    if (t.id==='loadProfileBtn'){
      ...
      state = migrateShape(JSON.parse(JSON.stringify(profiles[name]))); // por si cambia el esquema
