// === Altervenator v13s3 — Diaria + Clase + Urgentes ===
// - Diaria (auto-aceptada)
// - Clase (aceptar/rechazar, sin penalización al fallar, XP de clase)
// - Urgentes (máx 3/semana ISO, 5h, no se pueden rechazar, overlay rojo, 20% loot raro)
// - Penalización Urgente al fallar: -10🪙 + Nerf EXP + "Versión dura" (+25%, 6h, sin penalización)
// - Temporizadores + auto-fail
// - Barra XP y datos Perfil actualizados
(function(){
  ...
  // ---------- Datos ----------
  var TYPE={DAILY:'daily', CLASS:'class', URGENT:'urgent'};
  ...
  function harderClone(m){
    ...
    n.penalty=null; // sin penalización en la versión dura
  ...
  function completeMission(m){
    ...
    showInfo('Misión completada','Has ganado +'+(m.baseXP||0)+' XP y +'+(m.baseCoins||0)+'🪙'+extra, m.type===TYPE.CLASS?'purple': (m.type===TYPE.URGENT?'red':'blue'));
  }
  ...
  function missionCard(m){
    ...
    var typeLabel = m.type===TYPE.DAILY ? 'Diaria' : (m.type===TYPE.CLASS?'Clase':'Urgente');
  ...
  function renderMissions(){
    ...
    var pend=state.missions.filter(function(x){return x.status==='pending' && (x.type===TYPE.DAILY||x.type===TYPE.CLASS||x.type===TYPE.URGENT);});
    var hist=state.missions.filter(function(x){return x.status!=='pending' && (x.type===TYPE.DAILY||x.type===TYPE.CLASS||x.type===TYPE.URGENT);}).slice(0,8);
  ...
  document.body.addEventListener('click', function(e){
    ...
    if (id&&act){
      ...
      if (!m) return;
      if (act==='accept'){ if (m.type!==TYPE.URGENT){ m.accepted=true; save(); renderAll(); } return; }
      if (act==='reject'){ if (m.type!==TYPE.URGENT){ m.status='rejected'; save(); renderAll(); } return; }
  ...
  maybeUrgent();          // 25% prob. al abrir, respeta máx 3/semana
  renderAll();
