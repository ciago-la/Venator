// === Altervenator app.v13s8.step3.js ===
// Compra → Inventario → Equipar/Quitar con PNGs reales.
// Mantiene todo lo ya implementado (diaria 1/día, focus/clase 2/día, pools aleatorios de clase,
// perfiles, tienda/consumibles, pity timer, notificaciones, XSS).

(function(){
  const VER='v13s8-step3';
  ...
  // --- imágenes reales (usa assets/* como te dije) ---
  const PNG={
  ...
  // migra a: classProgress (por-clase), cosmeticsOwned (array), equipped (array)
  function migrateShape(s){
    ...
    if (!Array.isArray(s.equipment)) s.equipment=[]; // legacy (equipados)
    if (!Array.isArray(s.cosmeticsOwned)) s.cosmeticsOwned=[]; // NUEVO (propiedad)
    ...
    if (!s.urgentPlan || typeof s.urgentPlan!=='object') s.urgentPlan={date:null,decided:false,willHave:false,fireAt:null,spawned:false};
    if (typeof s.daysWithoutUrgent!=='number') s.daysWithoutUrgent=0;
    if (!s.dailyCounters || typeof s.dailyCounters!=='object') s.dailyCounters={date:null, focusMade:0, classMade:0};

    // progreso por-clase
    if (!s.classProgress || typeof s.classProgress!=='object'){
      s.classProgress={}; 
      CLASSES.forEach(c=> s.classProgress[c]={level:1, xp:0});
    }
    // migración de classLevel/classXP antiguos
    ...
    // MIGRACIÓN: si tenías algo en equipment legacy, asegúrate de marcarlo también como Owned
    (s.equipment||[]).forEach(id=>{
      if (!s.cosmeticsOwned.includes(id)) s.cosmeticsOwned.push(id);
    });

    if (typeof s.schemaVersion!=='number' || s.schemaVersion<4) s.schemaVersion=4;
    return s;
  }
  ...
  function cxpNeed(){ return cxpNeedFor(classObj().level); }
  function gainClassXP(base){
    ...
    while(cp.xp>=cxpNeedFor(cp.level)){
      ...
      cp.level++;
      if (cp.level%10===0){
        const award=({Asesino:'botas_sombra',Guerrero:'guanteletes_coloso',Mago:'anillo_mana','Amigo del dragón':'simbolo_espiritu',Arquero:'talisman_ojo_agudo',Saltamontes:'magnesio_lava',Maratón:'estela_alba',Espía:'pendiente_fantasma'})[state.hero.cls]||'gema_rara';
        state.inventory[award]=(state.inventory[award]||0)+1;
      }
    }
  }
  ...
  // --- pools (diarias/clase) resumidos: iguales que versión anterior ---
  ...
  const CLASS_POOL_STRINGS={ /* igual que el step2b (recortado aquí por brevedad) */
  ...
  function mkDaily(){
    ...
    const reqs=DAILY_ROTATION[now.getDay()].map(s=>({label:scaleTextForLevel(s,state.level)}));
  ...
  function mkClassMission(cls){
    const now=new Date(); const pool=CLASS_POOL_STRINGS[cls]||['Técnica 1','Técnica 2','Técnica 3','Técnica 4']; 
    const cp=classObj(); const chosen=pickN(pool,2);
    return {id:uid(),type:TYPE.CLASS,title:'Misión de clase — '+cls,desc:'Entrenamiento específico de tu clase.',createdAt:now.toISOString(),dueAt:new Date(now.getTime()+12*3600*1000).toISOString(),status:'pending',accepted:false,baseXP:0,baseCoins:9,classXP:70,requirements:chosen.map(s=>({label:scaleTextForLevel(s,cp.level)})),penalty:null};
  }
  function mkUrgent(){
    const T=[
  ...
  // --- notificaciones (idéntico a step2b) ---
  ...
  // --- pity + límites + rollover (idénticos a step2b) ---
  ...
  function planUrgentForTodayIfNeeded(){
    ...
    if(used<3){
      ...
      if(will){ const h=3+Math.floor(Math.random()*17), m=Math.floor(Math.random()*60); const fire=new Date(); fire.setHours(h,m,0,0); plan.willHave=true; plan.fireAt=fire.toISOString(); }
    }
  ...
  function triggerScheduledUrgentIfTime(){
    ...
    if(now>due){ u.createdAt=new Date(fireAt).toISOString(); u.dueAt=new Date(due).toISOString(); u.status='failed';
      ...
      state.missions.unshift(u); state.weeklyUrgents[wk]=(state.weeklyUrgents[wk]||0)+1; p.spawned=true; save(); renderAll(); onUrgentSpawned(); notifyNow('⚠️ Urgente perdida','La misión urgente de hoy venció antes de entrar.'); return; }
    state.missions.unshift(u); state.weeklyUrgents[wk]=(state.weeklyUrgents[wk]||0)+1; p.spawned=true; save(); renderAll(); onUrgentSpawned(); notifyNow('⚡ ¡Misión urgente!','Tienes 5 horas.'); scheduleDueSoonNotification(u);
  }
  ...
  function completeMission(m){
    ...
    if(m.type===TYPE.URGENT && Math.random()<0.20 && m.loot && m.loot.length){ const item=m.loot[Math.floor(Math.random()*m.loot.length)]; state.inventory[item]=(state.inventory[item]||0)+1; showInfo('Objeto raro recibido','Has obtenido: '+item,'blue'); }
    save(); renderAll(); const extra=m.classXP?(' · +'+m.classXP+' XP clase'):''; const col=m.type===TYPE.CLASS?'purple':(m.type===TYPE.URGENT?'red':(m.type===TYPE.FOCUS?'blue':'blue')); showInfo('Misión completada','Has ganado +'+(m.baseXP||0)+' XP y +'+(m.baseCoins||0)+'🪙'+extra, col);
  }
  ...
  function renderShop(){
    ...
    SHOP.consumibles.forEach(it=>{
      const li=el('li','card');
      const row=el('div','itemrow'); row.appendChild(iconImgSmall(PNG.equip_gafas)); // icono genérico
    ...
    SHOP.esteticos.forEach(it=>{
      ...
      if (!owned){
        const b=el('button',null,'Comprar'); b.dataset.buy=it.id; btns.appendChild(b);
      }else{
        const tag=el('span','badge', isEquipped(it.id)?'Equipado':'En inventario'); btns.appendChild(tag);
        const be=el('button', null, isEquipped(it.id)?'Quitar':'Equipar'); be.dataset.cosm=it.id; be.dataset.toggle='1'; btns.appendChild(be);
      }
      li.appendChild(btns); shopEsteticos.appendChild(li);
    });

    // inventario (consumibles y equipo)
    // Consumibles con usar global
    ...
    if (state.cosmeticsOwned.length){
      ...
      state.cosmeticsOwned.forEach(id=>{
        ...
        row.appendChild(el('h4',null, meta.name));
        li.appendChild(row);
        const btns=el('div','btnrow');
        const be=el('button',null, isEquipped(id)?'Quitar':'Equipar'); be.dataset.cosm=id; be.dataset.toggle='1'; btns.appendChild(be);
        li.appendChild(btns); inventoryList.appendChild(li);
      });
    }
  ...
  function renderMissions(){
    ...
    head.appendChild(row); head.appendChild(el('div','small','⚡ Urgentes semana: '+used+'/3'));
  ...
  document.addEventListener('click', function(e){
    ...
    if (t.dataset.buy){
      ...
      if (id.startsWith('equip_')){
        if (!ownCosmetic(id)) state.cosmeticsOwned.push(id); // pasa a inventario de equipo
        showInfo('Adquirido','Se añadió a tu inventario: '+id.replace('equip_','').replace('_',' '),'blue');
      }else{
    ...
    // equipar / quitar equipo
    if (t.dataset.toggle==='1' && t.dataset.cosm){
      const id=t.dataset.cosm; if (!ownCosmetic(id)) return;
      if (isEquipped(id)) unequip(id); else equip(id);
      return;
    }

    // inventario global (consumibles)
  ...
  if (importProfileInput) importProfileInput.addEventListener('change', e=>{
    ...
    r.onload=function(){ try{ const data=JSON.parse(r.result); if(data && data.state){ state=migrateShape(data.state); save(); renderAll(); alert('Perfil importado: '+(data.name||'perfil')); } else alert('Archivo inválido'); }catch(_){ alert('No se pudo leer el archivo'); } };
    r.readAsText(f);
  });

  // --- Misiones únicas/auto + pity ---
  ...
  function ensureClassMissionIfNone(){
    ...
    if(!has){ const c=state.hero.cls||'Asesino'; const m=mkClassMission(c); state.missions.unshift(m); save(); }
  }
  ...
  function tick(){
    ...
    $$('#missionsList .card').forEach(card=>{
      ...
      const m=state.missions.find(x=>x.id===id); const tmr=card.querySelector('.timer');
