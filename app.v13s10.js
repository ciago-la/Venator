// === Altervenator app.v13s10.js ===
// Novedad: Llaves + "Asalto a mazmorra" (misión urgente forzada, mismo formato).
// - Ganas 1 llave al completar cada URGENTE.
// - Ganas 1 llave por cada 3 niveles subidos (acumulativo).
// - Botón "⚔️ Asalto a mazmorra (Llaves: X)" que consume 1 llave y crea una misión tipo mazmorra.
// Mantiene: diarias únicas, focus/clase (límite 2/día), pools aleatorios de clase,
// pity de urgentes, perfiles, tienda+inventario+equipo PNGs, overlays (verde/amarillo/rojo),
// notificación del Castigador cuando hay Versión dura, etc.
...
(function(){
  ...
  function migrateShape(s){
    ...
    // NUEVO: llaves de mazmorra
    if(typeof s.dungeonKeys!=='number') s.dungeonKeys=0;
    // nivel base para cálculo de llaves por niveles (guardamos el último nivel contabilizado)
  ...
  function gainXP(base){
    ...
    // Llaves por niveles: +1 llave por cada 3 niveles superados respecto al último check
  ...
  // NUEVO: mazmorra = copia de urgente pero tipo DUNGEON y título distinto
  function mkDungeon(){
    const u=mkUrgent();
    u.type=TYPE.DUNGEON;
  ...
  function completeMission(m){
    ...
    decayNerf();
    // Objeto raro en urgentes/mazmorra (20%)
    ...
    // Llave por completar URGENTE (no por mazmorra)
  ...
  function failMission(m){
    if(!m || m.status!=='pending') return;
    m.status='failed';
    let firedPunisher=false;
    if(m.penalty){
      if(m.penalty.coins) state.coins=Math.max(0,state.coins-m.penalty.coins);
      if(m.penalty.nerf) applyNerf();
      if(m.penalty.nextHarder){ state.missions.unshift(harderClone(m)); firedPunisher=true; }
    }
    save(); renderAll();
    if (firedPunisher) showPunisher('Has fallado '+m.title+'. Se activa la Versión dura durante 6 horas.');
  ...
  function renderShop(){
    ...
    if (state.cosmeticsOwned.length){
      ...
      state.cosmeticsOwned.forEach(id=>{
        ...
        const btns=el('div','btnrow'); const be=el('button',null, state.equipment.includes(id)?'Quitar':'Equipar'); be.dataset.cosm=id; be.dataset.toggle='1'; btns.appendChild(be); li.appendChild(btns);
  ...
  document.addEventListener('click', function(e){
    ...
    if (t.id==='newClassBtnSmall' || t.id==='newClassBtn'){
      ...
      const c=state.hero.cls||'Asesino'; const m=mkClassMission(c); state.missions.unshift(m); state.dailyCounters.classMade++; save(); renderAll(); showOverlay('purple','Nueva misión de clase','Tienes una misión: '+m.title+' — ¿Aceptas?',null); return;
    }

    // NUEVO: Asalto a mazmorra
    if (t.id==='dungeonBtn'){
    ...
    if (t.dataset.buy){
      ...
      if (state.coins<price) return showWarn('No tienes monedas suficientes.');
      state.coins-=price;
  ...
  // prompts aceptar/rechazar (solo para focus/clase)
