// === Altervenator app.v13s9.js ===
// Novedades:
// - Notificaciones estiladas en overlay: rojo/verde/amarillo (+ PNG opcional)
// - Al FALLAR una misión con 'nextHarder', aparece notificación ROJA del Castigador
// - Compra correcta: VERDE “X comprado… puedes verlo en inventario”
// - Sin dinero: AMARILLO “No tienes monedas suficientes”
//
// Mantiene todo lo anterior: diarias únicas, focus/clase 2/día, pools aleatorios, perfiles,
// tienda/consumibles, inventario/equipar, pity timer urgentes, notificaciones nativas, XSS básico.
...
(function(){
  ...
  // Banner rápido si hay errores JS
  ...
  // Prompts aceptar/rechazar (misiones)
  ...
  // --- Notificaciones nativas (sin cambios) ---
  ...
  function failMission(m){
    ...
    else showInfo('Misión fallida','Se aplicó la penalización.', m.type===TYPE.CLASS?'purple':(m.type===TYPE.URGENT?'red':'blue'));
  }
  ...
  document.addEventListener('click', function(e){
    ...
    if (idm&&act){
      ...
      if (act==='accept'){ if(m.type!=='urgent'){ m.accepted=true; save(); renderAll(); showSuccess('Has aceptado: '+m.title); } return; }
      if (act==='reject'){ if(m.type!=='urgent'){ m.status='rejected'; save(); renderAll(); showWarn('Has rechazado: '+m.title); } return; }
  ...
  // --- tick ---
  function tick(){ const now=Date.now(); let dirty=false; $$('#missionsList .card').forEach(card=>{ const id=card.getAttribute('data-id'); if(!id) return; const m=state.missions.find(x=>x.id===id); const tmr=card.querySelector('.timer'); if (m&&tmr&&m.dueAt) tmr.textContent=fmt(new Date(m.dueAt).getTime()-now); if (m && m.status==='pending' && m.dueAt && now>new Date(m.dueAt).getTime()){ m.status='failed'; let fired=false; if (m.penalty){ if (m.penalty.coins) state.coins=Math.max(0,state.coins-m.penalty.coins); if (m.penalty.nerf) applyNerf(); if (m.penalty.nextHarder){ state.missions.unshift(harderClone(m)); fired=true; } } dirty=true; if (fired) showPunisher('Has dejado vencer '+m.title+'. Se activa la Versión dura.'); else showWarn('Misión vencida: '+m.title); } }); triggerScheduledUrgentIfTime(); if (dirty){ save(); renderAll(); } }
